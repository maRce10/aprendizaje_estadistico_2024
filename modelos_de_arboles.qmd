...
El árbol de regresión mostrado arriba es una sobre-simplificación de la verdadera relación entre hits, años y salario. Sin embargo, estos modelos pueden tener ventajas sobre otros tipos de modelos de regresión ya que es más fácil de interpretar y tiene una representación gráfica atractiva.
...
2.  **Aleatoriedad en la construcción de los árboles**

    -   Seleccionan subconjuntos aleatorios de datos para construir cada árbol (bootstrap sampling) e introducen aleatoriedad adicional en la selección de las variables que se evalúan en cada división.

3.  **Reducción de la varianza**: combinan las predicciones de múltiples árboles para promediar (regresión) o votar (clasificación), reduciendo la varianza del modelo.

4.  **Uso de hiperparámetros**: Los hiperparámetros en los modelos de aprendizaje estadístico son configuraciones o valores que controlan el comportamiento de el modelo antes del proceso de entrenamiento. A diferencia de los parámetros del modelo, que se ajustan automáticamente durante el entrenamiento (por ejemplo, los coeficientes en la regresión lineal), los hiperparámetros deben definirse o ajustarse manualmente antes del entrenamiento. Todos estos métodos requieren la configuración de hiperparámetros para controlar aspectos como:

    -   Número de árboles.
    -   Profundidad máxima de los árboles.
    -   Fracción de muestras o variables utilizadas por árbol.

5.  **Manejo de alta dimensionalidad**: Son efectivos en conjuntos de datos con un gran número de variables predictoras, aprovechando su capacidad para identificar las más relevantes.

6.  **Robustez frente al sobreajuste**: La combinación de múltiples árboles hace que estos métodos sean menos propensos al sobreajuste en comparación con un solo árbol. Sin embargo, boosting puede sobreajustar si no se ajustan correctamente los hiperparámetros.
...
1.  **Submuestreo de características (subsample)**: Define la proporción de muestras que se usarán para entrenar cada árbol. La submuestreo ayuda a reducir el sobreajuste, especialmente cuando se tienen muchos datos. Valor típico: 0.5-1.0.
